  ...
  public:
    ...
    while(p1||p2)
    {
        ...
        else if(!p2)
        {
           ...
           int pw1=p1->pow,pw2=p2->pow;
            power=max(pw1,pw2);
           coeff=(p1->coeff)*(pw1>=pw2)+(p2->coeff)*(pw2>=pw1);
          if(pw1>=pw2) p1=p1->next;
          if(pw2>=pw1) p2=p2->next;
            
        }
        if(!head)
        {
            head=sum=new Node(coeff,power);
            
        }
        else
        {
            sum->next=new Node(coeff,power);
            sum=sum->next;
        }
    }
    return head;
